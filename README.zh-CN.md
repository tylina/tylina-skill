...
> 共同编辑需要应用授权的连接；DSH WASM 0.4.10 已提供该入口。
...
macOS 打包版也已验证配置导出、真实编译与渲染、编辑、撤销授权和 Undo。
其它客户端和平台仍需分别验收。
...
在桌面版 Agent 侧栏点击插头图标 **连接外部 Agent**，导出 MCP 配置。
无需创建内置 AI 会话；把配置文件保存在文档项目以外：

```sh
tylina mcp --connection /path/to/private/mcp.json
```

同一入口可撤销连接，已接受的修改与 Undo 不受影响；关闭编辑器窗口也会撤销授权。
