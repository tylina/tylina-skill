...
> **Standalone pre-processing workflow** — produces source materials that can feed into the main Touying slide generation pipeline or be used independently.
...
- [Topic Confirmation](#step-1-topic-confirmation)
- [Content Research](#step-2-content-research)
- [Image Collection](#step-3-image-collection)
- [Output Summary](#step-4-output-summary)
- [Notes](#notes)
...
All outputs MUST be placed under the project's `sources/` directory:
...
- Aim for slide-ready content density: enough detail to fill 10-20 slides, but not an exhaustive research paper

### 2.3 Save Document

Save the Markdown document to:

```
<project>/sources/topic-research.md
```
...
| Criterion | Guideline |
|-----------|-----------|
| **Quantity** | 6-12 images (enough to support a 10-20 page presentation) |
...
**Topic**: <topic_name>
**Document**: `<project>/sources/topic-research.md` — [X sections, approximately Y words]
**Images**: `<project>/sources/topic-research-assets/` — [N images collected]
...
> From here, the user or main pipeline can use these materials as input for Touying slide generation (the document is already in the project's sources/ directory).

---

## Notes

- This workflow is **content-gathering only** — it does not create a Touying project, generate Typst code, or produce a design spec
- The Markdown document should be **slide-ready**: well-structured, factual, with clear sections that map naturally to presentation slides
