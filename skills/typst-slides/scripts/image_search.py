...
ZERO_CONFIG_PROVIDERS: tuple[str, ...] = ("wikimedia", "openverse")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Pin one provider. Default: try zero-config providers (wikimedia, "
            "openverse) plus any keyed provider whose API key is set."
        ),
    )
    parser.add_argument(
        "--orientation",
...
def _default_provider_chain() -> list[str]:
    """Keyed high-quality providers first; then zero-config providers
    (wikimedia primary, openverse fallback). This is the search order
