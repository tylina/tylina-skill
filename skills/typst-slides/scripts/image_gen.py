...
Usage:
  ...
  python3 image_gen.py --backend openrouter "a beautiful landscape"
"""
...
BACKEND_REGISTRY = {
    ...
    "qwen": {
        ...
        "key_env_vars": ["QWEN_API_KEY", "DASHSCOPE_API_KEY"],
    ...
    "zhipu": {
        ...
        "key_env_vars": ["ZHIPU_API_KEY", "BIGMODEL_API_KEY"],
    ...
    "volcengine": {
        ...
        "key_env_vars": ["VOLCENGINE_API_KEY", "ARK_API_KEY"],
    ...
    "modelscope": {
        ...
        "key_env_vars": ["MODELSCOPE_API_KEY"],
    ...
    "stability": {
        ...
        "key_env_vars": ["STABILITY_API_KEY"],
    ...
    "siliconflow": {
        ...
        "key_env_vars": ["SILICONFLOW_API_KEY"],
        "aliases": ["silicon"],
    },
    "fal": {
        "module": "backend_fal",
        ...
        "key_env_vars": ["FAL_KEY", "FAL_API_KEY"],
        "aliases": ["fal-ai"],
    },
    "replicate": {
        ...
        "key_env_vars": ["REPLICATE_API_TOKEN", "REPLICATE_API_KEY"],
    },
    "openrouter": {
...
def _has_api_key(info: dict) -> bool:
    """Check whether any of a backend's API key env vars are set and non-empty."""
    return any(os.environ.get(v) for v in info["key_env_vars"])


def _check_available() -> None:
    """Check which backends have API keys configured and print a summary."""
    ready = []
    not_ready = []
    for name, info in sorted(
        ...
        if _has_api_key(info):
            ready.append((name, info))
        else:
            not_ready.append((name, info))

    if ready:
        print(f"Available backends ({len(ready)}):\n")
        for name, info in ready:
            print(f"  [{info['tier']}] {name:<12} {info['label']} (model: {info['default_model']})")
        print()
    else:
        print("No backends available (no API keys configured).\n")

    if not_ready:
        print(f"Not configured ({len(not_ready)}):\n")
        for name, info in not_ready:
            print(f"  {name:<12} needs: {info['key_hint']}")
        print()

    print(f"Config file: {ENV_PATH}")
    if ready:
        print(f"\nTo generate: python3 image_gen.py \"prompt\" --backend {ready[0][0]}")
    else:
        print(
            "\nTo configure a backend, set IMAGE_BACKEND and the provider API key in\n"
            f"  {ENV_PATH}\n"
...
def main() -> None:
    ...
    parser.add_argument(
        "--check-available", action="store_true",
        help="Check which backends have API keys configured and exit."
    )
    parser.add_argument(
    ...
    if args.check_available:
        try:
            ...
            pass  # still check what's available
        _check_available()
        return
