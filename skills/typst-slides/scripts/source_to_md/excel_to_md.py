...
def _report_result(out_file: Path, warnings: list[str] | None = None) -> None:
...
def _convert_excel(
    ...
    if not visible_sheets:
        ...
        if warnings is not None:
            warnings.append("Workbook contains no visible sheets")
    ...
    for worksheet in visible_sheets:
        ...
        if bounds is None:
            ...
            if warnings is not None:
                warnings.append(f"Sheet '{worksheet.title}' contains no cell content")
            continue
        ...
        if rows_truncated or cols_truncated:
            ...
            if warnings is not None:
                warnings.append(
                    f"Sheet '{worksheet.title}' was truncated: {', '.join(limit_notes)}"
                )
...
def convert_to_markdown(
    ...
    *,
    ...
    try:
        ...
        print(f"[ERROR] Excel conversion failed: {exc}")
        return ""
    ...
    try:
        ...
        try:
            ...
            print(f"[ERROR] Excel receipt cleanup failed: {cleanup_exc}")
        finally:
    ...
    transaction.__exit__()
    _report_result(out_file, warnings)
    return markdown
